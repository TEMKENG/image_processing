...
void pr(const std::string str,c_color color,c_decoration decoration=c_decoration::NORMAL){
  std::cout<<"\033["<<decoration<<";"<<color<<"m"<<str<<"\033[0m";
}

void prl(const std::string str,c_color color,c_decoration decoration=c_decoration::NORMAL){
...
Histogram::Histogram(int _rows, int _cols, int channel, const uint8_t *data_) : Histogram() {
    ...
    for (int i = 0; i < MAX; ++i) {
        ...
        variance += i * i * data[i];
        if (data[i] != 0) dynamic++;
        if (min_ == -1 && data[i] != 0) min_ = i; // Smallest pixel value
...
Image::Type Image::getType(const char *filename) {
    ...
    if (strcmp(extension, ".jpg") == 0 || strcmp(extension, ".jpeg") == 0) return JPG;
...
float Image::at(int row, int col, uint8_t color) {
    ...
    // return data[(row * cols + col) * 1 + color];
}
